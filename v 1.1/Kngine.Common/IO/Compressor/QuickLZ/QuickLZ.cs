...
// Implemented by Kngine Infra. Team 2011 - this compression is porting for the C version of the QuickLZ compression algorithm

using System;
using System.Collections.Generic;
using System.Text;

namespace Kngine.IO.Compression
{

    /// <summary>
    /// Quick LZ is compreassion algorithm that provide low latency / High performance comprassion compare the other. zlib: 55 MB/s, Deflator 90 MB/s, MS .NET 75 MB/s, Fast LZ around LZO 100 MB/s
    /// This class ports of the C version of the Quick LZ. 
    /// This implments is doesn't use any unsafe code and it could be faster. Now it speed : 135MB/s for Compression & Decompression
    /// </summary>
    public static class QuickLZ
    {
        public const int cQLZ_STREAMING_BUFFER = 0;

        public const int cQLZ_MEMORY_SAFE = 0;

        private const int cHASH_VALUES = 4096;
        private const int cMINOFFSET = 2;
        private const int cUNCONDITIONAL_MATCHLEN = 6;
        private const int cUNCOMPRESSED_END = 4;
        private const int cCWORD_LEN = 4;
        private const int cDEFAULT_HEADERLEN = 9;
        private const int cQLZ_POINTERS_1 = 1;
        private const int cQLZ_POINTERS_3 = 16;
        ...
        public static byte[] Decompress(byte[] source)
        {
            try
            {
                return InternalDecompress(source);
            }
            catch
            {
                return source;
        ...
        public static byte[] InternalDecompress(byte[] source)
        {
            int level;
            ...
            int last_matchstart = size - cUNCONDITIONAL_MATCHLEN - cUNCOMPRESSED_END - 1;
            int last_hashed = -1;
            int hash;
            uint fetch = 0;
            ...
            for (; ; )
            {
                ...
                if ((cword_val & 1) == 1)
                {
                    ...
                    if (level == 1)
                    {
                        ...
                        {
                            last_hashed++;
                            hash = (int)(((fetch >> 12) ^ fetch) & (cHASH_VALUES - 1));
                    ...
                    if (dst <= last_matchstart)
                    {
                        ...
                        if (level == 1)
                        {
                            while (last_hashed < dst - 3)
                            {
                                ...
                                hash = ((fetch2 >> 12) ^ fetch2) & (cHASH_VALUES - 1);
                                hashtable[hash] = last_hashed;
                                hash_counter[hash] = 1;
                            }
        ...
        public static byte[] Compress(byte[] source)
        {
            try
            {
                return InternalCompress(source);
            }
            catch
            {
                return source;
        ...
        public static byte[] InternalCompress(byte[] source)
        {
            if (source == null || source.Length == 0) return new byte[0];

            int src = 0;
            int dst = cDEFAULT_HEADERLEN + cCWORD_LEN;
            uint cword_val = 0x80000000;
            int cword_ptr = cDEFAULT_HEADERLEN;
            byte[] destination = new byte[source.Length + 400];
            int[,] hashtable;
            int[] cachetable = new int[cHASH_VALUES];
            byte[] hash_counter = new byte[cHASH_VALUES];
            byte[] d2;
            int fetch = 0;
            int last_matchstart = (source.Length - cUNCONDITIONAL_MATCHLEN - cUNCOMPRESSED_END - 1);
            int lits = 0;
            int level = 1;
            hashtable = new int[cHASH_VALUES, cQLZ_POINTERS_1];
            ...
            while (src <= last_matchstart)
            {
                if ((cword_val & 1) == 1)
                {
                    ...
                    {
                        d2 = new byte[source.Length + cDEFAULT_HEADERLEN];
                        write_header(d2, level, false, source.Length, source.Length + cDEFAULT_HEADERLEN);
                        System.Array.Copy(source, 0, d2, cDEFAULT_HEADERLEN, source.Length);
                        return d2;
                    }
                ...
                if (level == 1)
                {
                    int hash = ((fetch >> 12) ^ fetch) & (cHASH_VALUES - 1);
                    int o = hashtable[hash, 0];
                    int cache = cachetable[hash] ^ fetch;
                    cachetable[hash] = fetch;
                    hashtable[hash, 0] = src;

                    if (cache == 0 && hash_counter[hash] != 0 && (src - o > cMINOFFSET || (src == o + 1 && lits >= 3 && src > 3 && source[src] == source[src - 3] && source[src] == source[src - 2] && source[src] == source[src - 1] && source[src] == source[src + 1] && source[src] == source[src + 2])))
                    {
                        ...
                        {
                            int old_src = src;
                            int remaining = ((source.Length - cUNCOMPRESSED_END - src + 1 - 1) > 255 ? 255 : (source.Length - cUNCOMPRESSED_END - src + 1 - 1));
                ...
                else
                {
                    ...
                    matchlen = 0;
                    offset2 = 0;
                    for (k = 0; k < cQLZ_POINTERS_3 && c > k; k++)
                    {
                        o = hashtable[hash, k];
                        if ((byte)fetch == source[o] && (byte)(fetch >> 8) == source[o + 1] && (byte)(fetch >> 16) == source[o + 2] && o < src - cMINOFFSET)
                        {
                            m = 3;
                    ...
                    o = offset2;
                    hashtable[hash, c & (cQLZ_POINTERS_3 - 1)] = src;
                    c++;
                    hash_counter[hash] = c;
                    ...
                    if (matchlen >= 3 && src - o < 131071)
                    {
                        ...
                        for (int u = 1; u < matchlen; u++)
                        {
                            ...
                            hash = ((fetch >> 12) ^ fetch) & (cHASH_VALUES - 1);
                            c = hash_counter[hash]++;
                            hashtable[hash, c & (cQLZ_POINTERS_3 - 1)] = src + u;
                        }
            ...
            fast_write(destination, cword_ptr, (int)((cword_val >> 1) | 0x80000000), cCWORD_LEN);
        ...
        public static int UnsafeInternalCompress(byte[] source, out byte[] d2)
        {
            d2 = null;
